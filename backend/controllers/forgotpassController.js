      ...
      const mailOptions = {
        ...
        html: `
          <div style="font-family: Arial, sans-serif; padding: 20px;">
            ...
            <a href="http://localhost:1234/resetForm/${forpasswordrequest.id}" style="display: inline-block; padding: 10px 20px; background-color: #007bff; color: #fff; text-decoration: none; border-radius: 5px;">Reset Password</a>
...
const resetpassword = async (req, res, next) => {
  try {
    ...
    res.redirect(`http://localhost:1234/resetForm?uuid=${req.params.uuid}`);
  } catch (err) {
...
const newpassword = async (req, res, next) => {
  ...
  try {
    ...
    const forpasswordrequest = await forgotPasswordRequest.findOne({ where: { id: uuid, isactive: true } });
